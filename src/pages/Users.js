...
import { users_store } from "../redux/actions/users/users";
...
export default function Users() {
    ...
    const handleRemove = user => {
        setLoading(true);
        (async () => {
            const result = await API.removeUsers(user);
    ...
    const handleEnable = user => {
        setLoading(true);
        (async () => {
            const result = await API.enableUsers(user);
            checkResponse(result);
            setLoading(false);
            reload();
        })();
    ...
    const handleBlock = user => {
        setLoading(true);
        (async () => {
            const result = await API.blockUsers(user);
            checkResponse(result);
            setLoading(false);
            reload();
        })();
    ...
    const formActions = {
        ...
        add : async (data) => {
            ...
            const result = await API.addNewUser(data);
            checkResponse(result);
            handleCloseDialog();
            reload();
        }
    ...
    const depositActions = {
        apply : async (data) => {
            ...
            const result = await API.depositUser(data);
            checkResponse(result);
    ...
    const columns = [
        ...
        { 
            ...
            width: 150,
            renderCell: ({row}) => (
                <Chip label={row.userRole ? row.userRole.name : "None"} size="small" color="primary" />
            )
        }
    ];
